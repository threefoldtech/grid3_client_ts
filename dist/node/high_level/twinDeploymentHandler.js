...
class TwinDeploymentHandler {
    ...
    async createNameContract(name) {
        ...
        if (!c) {
            try {
                ...
                events_1.events.emit("logs", `Name contract with id: ${contract["contract_id"]} has been created`);
                return contract;
            }
            catch (e) {
    ...
    async update(deployment) {
        ...
        let contract;
        try {
            contract = await this.tfclient.contracts.updateNode(deployment.contract_id, "", deployment.challenge_hash());
        ...
        try {
            ...
            throw Error(`Failed to update deployment on node ${node_id} with contract ${contract["contract_id"]} due to ${e}`);
        }
        return contract;
    }
    ...
    async handle(twinDeployments) {
        ...
        try {
            for (const twinDeployment of twinDeployments) {
                ...
                if (twinDeployment.operation === models_1.Operations.deploy) {
                    twinDeployment.deployment.sign(this.config.twinId, this.config.mnemonic, this.tfclient.keypairType, "", this.config.signer);
                    ...
                    events_1.events.emit("logs", `A deployment has been created on node_id: ${twinDeployment.nodeId} with contract_id: ${contract["contract_id"]}`);
                }
                    ...
                    events_1.events.emit("logs", `Deployment has been updated with contract_id: ${contract["contract_id"]}`);
                }
